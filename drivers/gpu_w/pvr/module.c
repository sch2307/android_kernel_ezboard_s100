...
#ifdef CONFIG_FALINUX_ZEROBOOT
static int pvr_snapshot   = 0;
static int flag_sgx_init  = 0;

static DECLARE_WAIT_QUEUE_HEAD(zb_waitq);
static DECLARE_WAIT_QUEUE_HEAD(zb_waitq_locker_pvrsrv);		
static DECLARE_WAIT_QUEUE_HEAD(zb_waitq_locker_surfaceflinger);		

void zb_pvr_wakeup( void );
void zb_notify_by( int who, unsigned int seq );
int __init setup_pvr_snapshot( char *s );
extern int s3c_displayclass_init(void);
...
int __init setup_pvr_snapshot( char *s )
{
	pvr_snapshot = simple_strtoul( s, NULL, 0 );
	return 0;	
}
__setup( "pvr_snapshot=", setup_pvr_snapshot );
...
void zb_notify_by( int who, unsigned int seq )
{
	switch( who )
	{
	case 10 : // pvr_workqueue
		{
			if ( 0 == seq )
			{
				printk( "(notify)wakeup surfaceflinger--------------%s(%d)  pname=%s(%d)\n", __func__,__LINE__, current->comm, current->pid );
				pvr_snapshot = 0;
				wake_up_interruptible( &zb_waitq_locker_pvrsrv );
			}
			//else
			//{
			//	if ( 0 == (seq%10) ) printk( " pvr_workq callcnt=%d\n", seq );
			//}
		}
		break;
	}		
...
#endif
{
	...
	if ( pvr_snapshot  )
	{
		...
		{
			// wait for snapshot
			printk( "PVR: pname=%s(%d) open(waiting snapshot)----------%s(%d)\n", current->comm, current->pid, __func__, __LINE__ );
			interruptible_sleep_on_timeout( &zb_waitq, HZ*(10+pvr_snapshot) );
			printk( "PVR: pname=%s(%d) open(wakeup by NAL)-------------%s(%d)\n", current->comm, current->pid, __func__, __LINE__ ); // , current->parent->pid );
		}
		else if ( 0 == strcmp( current->comm, "SurfaceFlinger" ) )
		{
			// wait for pvrsrvinit
			printk( "PVR: pname=%s(%d) open(waiting pvrsrvinit)--------%s(%d)\n", current->comm, current->pid, __func__, __LINE__ );
			printk( "ready snapshot =========================================================\n" );		
			interruptible_sleep_on( &zb_waitq_locker_pvrsrv );
			printk( "PVR: pname=%s(%d) open(wakeup by pvrsrvinit)------%s(%d)\n", current->comm, current->pid, __func__, __LINE__ ); // , current->parent->pid );
		}
		else
		{
			printk( "PVR: what??? open(%s)--------------------%s(%d)  pname=%s(%d)\n", current->comm, __func__,__LINE__, current->comm, current->pid );		
			interruptible_sleep_on( &zb_waitq_locker_pvrsrv );
		}
	}
	
	// init hardware
	if ( ( 0 == flag_sgx_init ) && ( 0 == strcmp( current->comm, "pvrsrvinit" ) ) )
	{
			flag_sgx_init ++;
			if ( SysInitialise() != PVRSRV_OK )
				printk( "fatal error SysInitialise .......................\n" );		
			if ( s3c_displayclass_init() )
				printk( "fatal error s3c_displayclass_init ...............\n" );	
	}
#endif
...
//printk( "open(exit)---------------------%s(%d)  pname=%s(%d)\n", __func__,__LINE__, current->comm, current->pid );
	return iRet;
}
