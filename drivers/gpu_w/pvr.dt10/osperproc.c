#include	"DT_osperproc.h"	/* For DynamicTracer-TestPoint */
/**********************************************************************
 *
...
PVRSRV_ERROR OSPerProcessPrivateDataInit(IMG_HANDLE *phOsPrivateData)
{
	...
	__DtTestPoint(__DtFunc_OSPerProcessPrivateDataInit, __DtStep_0)
	...
	if (eError != PVRSRV_OK)
	{
		__DtTestPoint(__DtFunc_OSPerProcessPrivateDataInit, __DtStep_1)
		*phOsPrivateData = IMG_NULL;

		PVR_DPF((PVR_DBG_ERROR, "%s: OSAllocMem failed (%d)", __FUNCTION__, eError));
		__DtTestPoint(__DtFunc_OSPerProcessPrivateDataInit, __DtStep_2)
		return eError;
	}
...
PVRSRV_ERROR OSPerProcessPrivateDataDeInit(IMG_HANDLE hOsPrivateData)
{
	...
	__DtTestPoint(__DtFunc_OSPerProcessPrivateDataDeInit, __DtStep_0)
	if (hOsPrivateData == IMG_NULL)
	{
		__DtTestPoint(__DtFunc_OSPerProcessPrivateDataDeInit, __DtStep_1)
		return PVRSRV_OK;
	}
	...
	if (eError != PVRSRV_OK)
	{
		__DtTestPoint(__DtFunc_OSPerProcessPrivateDataDeInit, __DtStep_2)
		PVR_DPF((PVR_DBG_ERROR, "%s: OSFreeMem failed (%d)", __FUNCTION__, eError));
	}

	__DtTestPoint(__DtFunc_OSPerProcessPrivateDataDeInit, __DtStep_3)
	return PVRSRV_OK;
}

PVRSRV_ERROR OSPerProcessSetHandleOptions(PVRSRV_HANDLE_BASE *psHandleBase)
{
	__DtTestPoint(__DtFunc_OSPerProcessSetHandleOptions, __DtStep_0)
	return LinuxMMapPerProcessHandleOptions(psHandleBase);
}

IMG_HANDLE LinuxTerminatingProcessPrivateData(IMG_VOID)
{
	__DtTestPoint(__DtFunc_LinuxTerminatingProcessPrivateData, __DtStep_0)
	if(!gui32ReleasePID)
		return NULL;
	__DtTestPoint(__DtFunc_LinuxTerminatingProcessPrivateData, __DtStep_1)
