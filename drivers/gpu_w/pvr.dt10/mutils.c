#include	"DT_mutils.h"	/* For DynamicTracer-TestPoint */
/**********************************************************************
 *
...
pvr_pat_index(pgprotval_t prot_val)
{
	...
	__DtTestPoint(__DtFunc_pvr_pat_index, __DtStep_0)
	ret |= PROT_TO_PAT_INDEX(val, PAT);
	ret |= PROT_TO_PAT_INDEX(val, PCD);
	ret |= PROT_TO_PAT_INDEX(val, PWT);

	__DtTestPoint(__DtFunc_pvr_pat_index, __DtStep_1)
	return ret;
}
...
{
	__DtTestPoint(__DtFunc_pvr_pat_entry, __DtStep_0)
...
PVRLinuxX86PATProbe(IMG_VOID)
{
	...
	__DtTestPoint(__DtFunc_PVRLinuxX86PATProbe, __DtStep_0)
	if (cpu_has_pat)	 
	{
		u64 pat;
		IMG_UINT pat_index;
		...
		__DtTestPoint(__DtFunc_PVRLinuxX86PATProbe, __DtStep_1)
	...
	{
		__DtTestPoint(__DtFunc_PVRLinuxX86PATProbe, __DtStep_2)
		PVR_TRACE(("%s: Write combining available via PAT", __FUNCTION__));
	}
	else
	{
		__DtTestPoint(__DtFunc_PVRLinuxX86PATProbe, __DtStep_3)
...
#endif	
#endif	
	__DtTestPoint(__DtFunc_PVRLinuxX86PATProbe, __DtStep_4)
}
...
pvr_pgprot_writecombine(pgprot_t prot)
{
    ...
    __DtTestPoint(__DtFunc_pvr_pgprot_writecombine, __DtStep_0)
...
PVRLinuxMUtilsInit(IMG_VOID)
{
	...
	PVRLinuxX86PATProbe();
#endif
	__DtTestPoint(__DtFunc_PVRLinuxMUtilsInit, __DtStep_0)
}
