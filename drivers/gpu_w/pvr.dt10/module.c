...
#endif
{
	...
	if ( pvr_snapshot )
	{
		printk( "open(wait)---------------------------%s(%d)  pname=%s(%d)\n", __func__,__LINE__, current->comm, current->pid );
		interruptible_sleep_on( &zb_waitq );
		
		pvr_snapshot = 0;
		if ( 0 != strcmp( current->comm, "pvrsrvinit" ) )
		{
			interruptible_sleep_on_timeout( &zb_waitq, HZ );
		}

		printk( "open(wakeup)---------------------------%s(%d)  pname=%s(%d)\n", __func__,__LINE__, current->comm, current->pid );
	}
	else
	{
		printk( "open---------------------------%s(%d)  pname=%s(%d)\n", __func__,__LINE__, current->comm, current->pid );		
	}
#endif
