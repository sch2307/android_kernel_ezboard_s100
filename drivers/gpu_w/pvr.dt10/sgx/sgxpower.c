#include	"DT_sgxpower.h"	/* For DynamicTracer-TestPoint */
/**********************************************************************
 *
								...
								IMG_HANDLE				*phTimer)
{
	...
	__DtTestPoint(__DtFunc_SGXAddTimer, __DtStep_0)
	*phTimer = OSAddTimer(SGXOSTimer, psDeviceNode,
						  1000 * 50 / psSGXTimingInfo->ui32uKernelFreq);
	if(*phTimer == IMG_NULL)
	{
		__DtTestPoint(__DtFunc_SGXAddTimer, __DtStep_1)
		PVR_DPF((PVR_DBG_ERROR,"SGXAddTimer : Failed to register timer callback function"));
		__DtTestPoint(__DtFunc_SGXAddTimer, __DtStep_2)
		return PVRSRV_ERROR_OUT_OF_MEMORY;
	}

	__DtTestPoint(__DtFunc_SGXAddTimer, __DtStep_3)
	return PVRSRV_OK;
}
#endif 
...
static PVRSRV_ERROR SGXUpdateTimingInfo(PVRSRV_DEVICE_NODE	*psDeviceNode)
{
...
#if defined(SUPPORT_HW_RECOVERY)
	{
		...
		{
			__DtTestPoint(__DtFunc_SGXUpdateTimingInfo, __DtStep_0)
			...
			if (ui32OlduKernelFreq != psSGXTimingInfo->ui32uKernelFreq)
			{
				...
				__DtTestPoint(__DtFunc_SGXUpdateTimingInfo, __DtStep_1)
				eError = SGXAddTimer(psDeviceNode, psSGXTimingInfo, &hNewTimer);
				if (eError == PVRSRV_OK)
				{
					__DtTestPoint(__DtFunc_SGXUpdateTimingInfo, __DtStep_2)
					eError = OSRemoveTimer(psDevInfo->hTimer);
					if (eError != PVRSRV_OK)
					{
						__DtTestPoint(__DtFunc_SGXUpdateTimingInfo, __DtStep_3)
				...
				else
				{
					...
					__DtTestPoint(__DtFunc_SGXUpdateTimingInfo, __DtStep_4)
				}
			}
		}
		else
		{
			__DtTestPoint(__DtFunc_SGXUpdateTimingInfo, __DtStep_5)
			eError = SGXAddTimer(psDeviceNode, psSGXTimingInfo, &psDevInfo->hTimer);
			if (eError != PVRSRV_OK)
			{
				__DtTestPoint(__DtFunc_SGXUpdateTimingInfo, __DtStep_6)
				return eError;
			}
		}
	...
	{
		__DtTestPoint(__DtFunc_SGXUpdateTimingInfo, __DtStep_7)
	...
	{
		__DtTestPoint(__DtFunc_SGXUpdateTimingInfo, __DtStep_8)
...
static IMG_VOID SGXStartTimer(PVRSRV_SGXDEV_INFO	*psDevInfo)
{
	...
	if (eError != PVRSRV_OK)
	{
		__DtTestPoint(__DtFunc_SGXStartTimer, __DtStep_0)
		PVR_DPF((PVR_DBG_ERROR,"SGXStartTimer : Failed to enable host timer"));
	}
	#else
	PVR_UNREFERENCED_PARAMETER(psDevInfo);
	#endif 
	__DtTestPoint(__DtFunc_SGXStartTimer, __DtStep_1)
}
...
static IMG_VOID SGXPollForClockGating (PVRSRV_SGXDEV_INFO	*psDevInfo,
									   ...
									   IMG_CHAR				*pszComment)
{
	__DtTestPoint(__DtFunc_SGXPollForClockGating, __DtStep_0)
	PVR_UNREFERENCED_PARAMETER(psDevInfo);
	...
	if (PollForValueKM((IMG_UINT32 *)psDevInfo->pvRegsBaseKM + (ui32Register >> 2),
						...
						IMG_FALSE) != PVRSRV_OK)
	{
		__DtTestPoint(__DtFunc_SGXPollForClockGating, __DtStep_1)
	...
	__DtTestPoint(__DtFunc_SGXPollForClockGating, __DtStep_2)
}
...
{
	__DtTestPoint(__DtFunc_SGXPrePowerState, __DtStep_0)
		...
		(eNewPowerState != PVRSRV_DEV_POWER_STATE_ON))
	{
		...
		if (eError != PVRSRV_OK)
		{
			__DtTestPoint(__DtFunc_SGXPrePowerState, __DtStep_1)
			PVR_DPF((PVR_DBG_ERROR,"SGXPrePowerState: Failed to disable timer"));
			__DtTestPoint(__DtFunc_SGXPrePowerState, __DtStep_2)
			return eError;
		}
		#endif 
		...
		if (eNewPowerState == PVRSRV_DEV_POWER_STATE_OFF)
		{
			...
			__DtTestPoint(__DtFunc_SGXPrePowerState, __DtStep_3)
		...
		else
		{
			...
			__DtTestPoint(__DtFunc_SGXPrePowerState, __DtStep_4)
		...
		if (eError != PVRSRV_OK)
		{
			__DtTestPoint(__DtFunc_SGXPrePowerState, __DtStep_5)
			PVR_DPF((PVR_DBG_ERROR,"SGXPrePowerState: Failed to submit power down command"));
			__DtTestPoint(__DtFunc_SGXPrePowerState, __DtStep_6)
			return eError;
		}
		...
		if (PollForValueKM(&psDevInfo->psSGXHostCtl->ui32PowerStatus,
							...
							IMG_FALSE) != PVRSRV_OK)
		{
			__DtTestPoint(__DtFunc_SGXPrePowerState, __DtStep_7)
		...
		for (ui32Core = 0; ui32Core < SGX_FEATURE_MP_CORE_COUNT; ui32Core++)
		{
			...
			__DtTestPoint(__DtFunc_SGXPrePowerState, __DtStep_8)
		...
		if (eNewPowerState == PVRSRV_DEV_POWER_STATE_OFF)
		{
			...
			__DtTestPoint(__DtFunc_SGXPrePowerState, __DtStep_9)
			eError = SGXDeinitialise(psDevInfo);
			if (eError != PVRSRV_OK)
			{
				__DtTestPoint(__DtFunc_SGXPrePowerState, __DtStep_10)
				PVR_DPF((PVR_DBG_ERROR,"SGXPrePowerState: SGXDeinitialise failed: %u", eError));
				__DtTestPoint(__DtFunc_SGXPrePowerState, __DtStep_11)
				return eError;
			}
		}
	}

	__DtTestPoint(__DtFunc_SGXPrePowerState, __DtStep_12)
	return PVRSRV_OK;
}
...
PVRSRV_ERROR SGXPostPowerState (IMG_HANDLE				hDevHandle,
								...
								PVRSRV_DEV_POWER_STATE	eCurrentPowerState)
{
	__DtTestPoint(__DtFunc_SGXPostPowerState, __DtStep_0)
	if ((eNewPowerState != eCurrentPowerState) &&
		(eCurrentPowerState != PVRSRV_DEV_POWER_STATE_ON))
	{
		...
		__DtTestPoint(__DtFunc_SGXPostPowerState, __DtStep_1)
		...
		if (eCurrentPowerState == PVRSRV_DEV_POWER_STATE_OFF)
		{
			...
			__DtTestPoint(__DtFunc_SGXPostPowerState, __DtStep_2)
			eError = SGXUpdateTimingInfo(psDeviceNode);
			if (eError != PVRSRV_OK)
			{
				__DtTestPoint(__DtFunc_SGXPostPowerState, __DtStep_3)
				PVR_DPF((PVR_DBG_ERROR,"SGXPostPowerState: SGXUpdateTimingInfo failed"));
				__DtTestPoint(__DtFunc_SGXPostPowerState, __DtStep_4)
				return eError;
			}
			...
			if (eError != PVRSRV_OK)
			{
				__DtTestPoint(__DtFunc_SGXPostPowerState, __DtStep_5)
				PVR_DPF((PVR_DBG_ERROR,"SGXPostPowerState: SGXInitialise failed"));
				__DtTestPoint(__DtFunc_SGXPostPowerState, __DtStep_6)
				return eError;
			}
		}
		else
	...
	__DtTestPoint(__DtFunc_SGXPostPowerState, __DtStep_10)
	return PVRSRV_OK;
}
									 ...
									 PVRSRV_DEV_POWER_STATE	eCurrentPowerState)
{
	...
	__DtTestPoint(__DtFunc_SGXPreClockSpeedChange, __DtStep_0)
	PVR_UNREFERENCED_PARAMETER(psDevInfo);

	if (eCurrentPowerState == PVRSRV_DEV_POWER_STATE_ON)
	{
		__DtTestPoint(__DtFunc_SGXPreClockSpeedChange, __DtStep_1)
		if (bIdleDevice)
		{
			
			__DtTestPoint(__DtFunc_SGXPreClockSpeedChange, __DtStep_2)
			PDUMPSUSPEND();
			...
			if (eError != PVRSRV_OK)
			{
				__DtTestPoint(__DtFunc_SGXPreClockSpeedChange, __DtStep_3)
				PDUMPRESUME();
				__DtTestPoint(__DtFunc_SGXPreClockSpeedChange, __DtStep_4)
				return eError;
			}
		}
	}
	...
	__DtTestPoint(__DtFunc_SGXPreClockSpeedChange, __DtStep_5)
	return PVRSRV_OK;
}
									  ...
									  PVRSRV_DEV_POWER_STATE	eCurrentPowerState)
{
	...
	__DtTestPoint(__DtFunc_SGXPostClockSpeedChange, __DtStep_0)
	...
	if (eCurrentPowerState == PVRSRV_DEV_POWER_STATE_ON)
	{
		...
		__DtTestPoint(__DtFunc_SGXPostClockSpeedChange, __DtStep_1)
		eError = SGXUpdateTimingInfo(psDeviceNode);
		if (eError != PVRSRV_OK)
		{
			__DtTestPoint(__DtFunc_SGXPostClockSpeedChange, __DtStep_2)
			PVR_DPF((PVR_DBG_ERROR,"SGXPostPowerState: SGXUpdateTimingInfo failed"));
			__DtTestPoint(__DtFunc_SGXPostClockSpeedChange, __DtStep_3)
			return eError;
		}
		...
		if (bIdleDevice)
		{
			...
			__DtTestPoint(__DtFunc_SGXPostClockSpeedChange, __DtStep_4)
			...
			if (eError != PVRSRV_OK)
			{
				__DtTestPoint(__DtFunc_SGXPostClockSpeedChange, __DtStep_5)
				return eError;
			}
		}
		else
		{
			__DtTestPoint(__DtFunc_SGXPostClockSpeedChange, __DtStep_6)
			SGXStartTimer(psDevInfo);
		}
	}
	...
	__DtTestPoint(__DtFunc_SGXPostClockSpeedChange, __DtStep_7)
	return PVRSRV_OK;
}
