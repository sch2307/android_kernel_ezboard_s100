...
printk( "wait---------------------------%s(%d)  pname=%s(%d)\n", __func__,__LINE__, current->comm, current->pid );
#ifdef CONFIG_FALINUX_ZEROBOOT
//	while( 0 < open_cnt )
//	{
//		interruptible_sleep_on_timeout( &open_wq, HZ );
//	}
//	open_cnt = 1;
printk( "get ---------------------------%s(%d)  pname=%s(%d)\n", __func__,__LINE__, current->comm, current->pid );
#endif
...
#ifdef CONFIG_FALINUX_ZEROBOOT
extern void zb_raw_sgx_init( void );		// driver/gpu/pvr/sgx/sgxpower.c
void zb_sgx_init( void );
void zb_sgx_init( void )
{
	//zb_raw_sgx_init();
}
EXPORT_SYMBOL(zb_sgx_init);
...
//void zero_pvr_suspend( void )
//void zero_pvr_resume( void)

static void zero_pvr_suspend( void )
{
	if (PVRSRVSetPowerStateKM(PVRSRV_SYS_POWER_STATE_D4) != PVRSRV_OK)	printk("----- suspend error\n");
	else																printk("----- suspend success\n");
}
static void zero_pvr_resume( void)
{
...
static int std_proc_write( struct file *file, const char __user *buffer, unsigned long count, void *data)
{
	...
	if ( strncmp( "power=", cmd, 6 ) == 0 )
	{   
		switch( cmd[6] )
		{
		case '0' : 	zero_pvr_suspend(); break;	
		case '1' : 	zero_pvr_resume();  break;	
		}
	}
...
static void std_register_proc( void )
{
	...
	procdir = create_proc_entry( "falinux_pvr", S_IFREG | S_IRUGO, 0);
...
// ifconfig eth0 192.168.10.243 up
// busybox mount -t nfs -o nolock 192.168.10.10:/nfs /mnt/nfs
// busybox cp -a /mnt/nfs/zeroboot/fg/s5pv210-android/* /mnt/obb/.; cd /mnt/obb
// echo "power=0" > /proc/falinux_pvr
