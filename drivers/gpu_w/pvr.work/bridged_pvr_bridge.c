					   ...
					   PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	if((OSProcHasPrivSrvInit() == IMG_FALSE) || PVRSRVGetInitServerState(PVRSRV_INIT_SERVER_RUNNING) )
#else	 
...
#endif
	{		
printk( "-----%s(%d)  PVRSRV_ERROR_SRV_CONNECT_FAILED  %d  %d  %d\n", __func__,__LINE__,
				(OSProcHasPrivSrvInit() == IMG_FALSE) ? 1:0,
				PVRSRVGetInitServerState(PVRSRV_INIT_SERVER_RUNNING) ? 1:0,
				PVRSRVGetInitServerState(PVRSRV_INIT_SERVER_RAN) ? 1:0  );
...
//CONFIG_FALINUX_ZEROBOOT
	// debug @@@@@@
	pvr_msg = ( 0 == strcmp( current->comm, "pvrsrvinit" ) ) ?  1 : 0;
	// todo 이곳에서 제로부트로 시작일 경우  psPerProc->bInitProcess = 0 으로 설정하여 초기화 하도록 한다.
	if ( pvr_msg ) printk( "\t\t\t\t %s(%d)\n", __func__,__LINE__);
...
//if ( pvr_msg ) printk( "\t\t\t\t (%d)\n",__LINE__);			
			...
			{
//if ( pvr_msg ) printk( "\t\t\t\t (%d)  ui32BridgeID=%d\n", __LINE__, ui32BridgeID);			
	...
	// debug @@@@@@
//	if ( pvr_msg ) printk( "\t\t\t\t (%d)  psPerProc->bInitProcess=%d\n",__LINE__, psPerProc->bInitProcess );
