...
PVRSRV_ERROR OSInstallMISR(IMG_VOID *pvSysData)
{
	...
	{ 
		if (psEnvData->bMISRInstalled)
		{
			...
			printk( " ..<fake>	bMISRInstalled   psWorkQueue=%p\n", psEnvData->psWorkQueue );
			FA_EXIT();
			return PVRSRV_OK;
		}	
	}
...
printk( "-----%s(%d) psEnvData->psWorkQueue=%p\n", __func__,__LINE__, psEnvData->psWorkQueue );
...
IMG_HANDLE OSAddTimer(PFN_TIMER_FUNC pfnTimerFunc, IMG_VOID *pvData, IMG_UINT32 ui32MsTimeout)
{
	...
	{	
		if ( ui32i == 1 )
		{
			psTimerCBData = &sTimers[0];
			printk(  "..<fake>        first Timer\n" );	
			ui32i = 0;
		}
	}
#endif
...
if ( 0 == strcmp( current->comm, "pvrsrvinit" ) )
{
	static int use_timer_cnt = 0;
	use_timer_cnt ++;
	printk( "-----%s(%d)  reg_timer_cnt=%d pfn=%p  ui32i=%d timer=%p\n", __func__,__LINE__, use_timer_cnt, pfnTimerFunc, ui32i, psTimerCBData );   
}	
...
//	if ( 0 == strcmp( current->comm, "pvrsrvinit" ) )
	{
		static int del_timer_cnt = 0;
		del_timer_cnt ++;
		printk( "-----%s(%d)  del_timer_cnt=%d\n", __func__,__LINE__, del_timer_cnt );   
	}	
