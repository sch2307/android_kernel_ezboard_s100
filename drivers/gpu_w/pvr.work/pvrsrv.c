...
printk( "-----%s(%d)  PVRSRVPerProcessDataInit\n", __func__,__LINE__ );	
...
printk( "-----%s(%d)  OSEventObjectCreate\n", __func__,__LINE__ );	
...
PVRSRV_ERROR PVRSRVFinaliseSystem_SetPowerState_AnyCb(PVRSRV_DEVICE_NODE *psDeviceNode)
{
	static PVRSRV_DEVICE_NODE *save_psDeviceNode = NULL;
	PVRSRV_ERROR eError;
	FA_ENTER();
	
	if ( NULL == psDeviceNode ) psDeviceNode = save_psDeviceNode;
	save_psDeviceNode = psDeviceNode;
	printk( "\t\tfor zeroboot psDeviceNode=%p  %s(%d)\n", save_psDeviceNode, __func__,__LINE__ );
#else
...
printk( "-----%s(%d)  need=%08x  cur=%08x  pname=%s\n", __func__,__LINE__, ui32Value, ui32ActualValue, current->comm );			
