...
printk( "-----@@@------------%s(%d)  psDeviceNode=%p  psDevInfo=%p  sDevId.ui32DeviceIndex=%d\n", __func__,__LINE__, psDeviceNode, psDevInfo, psDeviceNode->sDevId.ui32DeviceIndex );
...
printk( "-----%s(%d)  pvRegsCpuVBase=%p\n", __func__,__LINE__, psDevInfo->pvRegsBaseKM );		
	}
	else
	{
#ifdef CONFIG_FALINUX_ZEROBOOT
		static IMG_PVOID zb_save_pvRegsBaseKM = NULL;
		if ( pvr_already_init ) 
			psDevInfo->pvRegsBaseKM = zb_save_pvRegsBaseKM;
		else
#endif
...
#ifdef CONFIG_FALINUX_ZEROBOOT
		zb_save_pvRegsBaseKM = psDevInfo->pvRegsBaseKM;
printk( "-----%s(%d)  pvRegsCpuVBase=%p\n", __func__,__LINE__, psDevInfo->pvRegsBaseKM );		
#endif
...
printk( "-----%s(%d) sDevId.ui32DeviceIndex=%d\n", __func__,__LINE__, psDeviceNode->sDevId.ui32DeviceIndex );
...
printk( "-----xxxxxxxxxxxxxxxxx%s(%d) pname=%s\n", __func__,__LINE__, current->comm );
...
//printk( "###### %s(%d) timer_handle() bPoweredDown=%d  pvData=%p\n", __func__,__LINE__, bPoweredDown, pvData );
...
//printk( "######%s(%d) MISR_handle()\n", __func__,__LINE__ );
	...
	{
//printk( "-----%s(%d)  %ld  %ld\n", __func__,__LINE__ , (psSGXHostCtl->ui32InterruptFlags & PVRSRV_USSE_EDM_INTERRUPT_HWR), (psSGXHostCtl->ui32InterruptClearFlags & PVRSRV_USSE_EDM_INTERRUPT_HWR) );
