...
#endif
{
	...
	// debug @@@@@@
	pvr_msg = ( 0 == strcmp( current->comm, "pvrsrvinit" ) ) ?  1 : 0;
	//if ( pvr_msg ) printk( "----------%s(%d)  ioctlCmd=0x%08x  arg=0x%08x\n", __func__,__LINE__,ioctlCmd, (unsigned int)arg );
	...
	// debug @@@@@@
	DTAB_PRN( "----------%s(%d)  ioctlCmd=0x%08x  arg=0x%08x  cmd=0x%08x\n", __func__,__LINE__,ioctlCmd, (unsigned int)arg, cmd  );
	...
	{
//if ( pvr_msg ) printk( "\t\t\t (%d) PVRSRV_BRIDGE_CONNECT_SERVICES=%08x\n",__LINE__, cmd );
...
//if ( pvr_msg ) printk( "\t\t\t (%d) cmd=PVRSRV_BRIDGE_EXPORT_DEVICEMEM\n" ,__LINE__ );
		...
		case PVRSRV_BRIDGE_MAP_DEV_MEMORY:
		{
			...
			PVRSRV_FILE_PRIVATE_DATA *psPrivateData = PRIVATE_DATA(pFile);
//if ( pvr_msg ) printk( "\t\t\t (%d) cmd=PVRSRV_BRIDGE_MAP_DEV_MEMORY\n" ,__LINE__ );
		...
		{
			PVRSRV_FILE_PRIVATE_DATA *psPrivateData = PRIVATE_DATA(pFile);
//if ( pvr_msg ) printk( "\t\t\t (%d) cmd=default %08x\n", __LINE__, cmd );
...
//if ( pvr_msg ) printk( "\t\t\t (%d) cmd=PVRSRV_BRIDGE_MAP_DEV_MEMORY %08x\n" ,__LINE__, cmd );
			if (authenticated)
			{
				break;
			}
...
//if ( pvr_msg ) printk( "\t\t\t (%d) BridgedDispatchKM\n" ,__LINE__ );
	...
	switch(cmd)
	{
		...
		case PVRSRV_BRIDGE_EXPORT_DEVICEMEM:
		{
			...
			PVRSRV_FILE_PRIVATE_DATA *psPrivateData = PRIVATE_DATA(pFile);
//if ( pvr_msg ) printk( "\t\t\t (%d) cmd=PVRSRV_BRIDGE_EXPORT_DEVICEMEM %08x\n" ,__LINE__, cmd );
...
//if ( pvr_msg ) printk( "\t\t\t (%d) cmd=PVRSRV_BRIDGE_MAP_DEVICECLASS_MEMORY %08x\n", __LINE__, cmd );
			break;
		}
#endif 
