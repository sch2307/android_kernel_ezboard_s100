...
printk( "-----%s(%d)\n", __func__,__LINE__ );
	...
	if (bAllDevices || (ui32DeviceIndex == psPowerDevice->ui32DeviceIndex))
	{
		...
		if (psPowerDevice->eCurrentPowerState != eNewDevicePowerState)
		{
//printk( "___@@_____%s(%d)  ui32DeviceIndex=%d  eNewDevicePowerState=%d  eCurrentPowerState=%d\n", __func__,__LINE__,
...
//					psPowerDevice->eCurrentPowerState );
			...
			{
//printk( "___@@_____%s(%d)  psPowerDevice=%p  hDevCookie=%p  eNewDevicePowerState=%d  eCurrentPowerState=%d\n", __func__,__LINE__,
//					psPowerDevice,
